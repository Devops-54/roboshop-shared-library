...
def sonarChecks(){
        stage('Sonar Checks') {
           ...
           // sh 'sonar-scanner -Dsonar.host.url=http://172.31.74.222:9000 ${ARGS} -Dsonar.projectKey=${COMPONENT} -Dsonar.login=admin -Dsonar.password=password'
...
def artifacts() {
        ...
        stage('Validate Artifact Version') {
            env.UPLOAD_STATUS=sh(returnStdout: true, script: "curl -L -s http://172.31.82.95:8081/service/rest/repository/browse/${COMPONENT} | grep ${COMPONENT}-${TAG_NAME}.zip || true" )
            print UPLOAD_STATUS
        }                    
        ...
        if(env.UPLOAD_STATUS == "null") {
                ...
                stage('Upload Artifacts') {
                        withCredentials([usernamePassword(credentialsId: 'NEXUS', passwordVariable: 'NEXUS_PSW', usernameVariable: 'NEXUS_USR')]) {
                                ...
                                sh "curl -f -v -u ${NEXUS_USR}:${NEXUS_PSW} --upload-file ${COMPONENT}-${TAG_NAME}.zip  http://172.31.82.95:8081/repository/${COMPONENT}/${COMPONENT}-${TAG_NAME}.zip || true"
